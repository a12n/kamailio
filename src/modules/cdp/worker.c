...
extern unsigned int
		workerq_latency_threshold; /**<max delay for putting task into worker queue */
extern unsigned int
		workerq_length_threshold_percentage; /**< default threshold for worker queue length, percentage of max queue length */
/**
...
void worker_destroy()
{
	...
	if(tasks) {
		...
		for(i = 0; i < tasks->max; i++) {
			if(tasks->queue[i].msg)
				AAAFreeMessage(&(tasks->queue[i].msg));
			tasks->queue[i].msg = 0;
			tasks->queue[i].p = 0;
		}
...
void cb_remove(cdp_cb_t *cb)
{
	...
	if(x->prev)
		...
		callbacks->head = x->next;
	if(x->next)
		x->next->prev = x->prev;
	else
		callbacks->tail = x->prev;
