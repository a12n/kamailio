...
static int w_as_relay_t(struct sip_msg *msg, char *entry, char *foo)
{
	...
	if(new_tran == 1 && msg->REQ_METHOD == METHOD_ACK) {
		 ...
		 * }
		 */
	} else if(!(buffer = create_as_event_t(
						seas_f.tmb.t_gett(), msg, processor_id, &len, 0))) {
...
static int seas_init(void)
{
	...
	if(p == NULL || *p == 0) {
		...
		if(port != (char *)0
				&& (seas_listen_port = str2s(port, strlen(port), &mierr))
						   == 0) {
	...
	memset(unc_as_t, 0,
			2 * MAX_UNC_AS_NR
					* sizeof(struct
							unc_as)); //useless because unc_as_t is in bss?
	if(pipe(c_pipe) == -1) {
...
{
	if(seas_listen_ip != NULL
			&& seas_listen_ip != &(get_first_socket()->address))
		pkg_free(seas_listen_ip);
	return;
}
