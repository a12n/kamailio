...
char *action_names[] = {"NONE", "PROVISIONAL_REPLY", "FINAL_REPLY",
		"REPLY_FIN_DLG", "UAC_REQ", "AC_RES_FAIL", "STATELESS_MSG", "AC_CANCEL",
		"JAIN_PONG"};
...
static inline int print_sock_info(char *buffer, int wheremax, int *idx,
		struct socket_info *s, enum sip_protos type);
static inline int send_sockinfo(int fd);
static inline int add_new_as(
		int event_idx, int action_idx, struct as_entry *as);
...
static int open_server_sockets(
		struct ip_addr *address, unsigned short port, int *fd);
				...
				for(as = as_list; as; as = as->next) {
					...
					if(as->u.as.action_pid == chld) {
						for(i = 0; i < as_nr
							...
							;
						if(i == as_nr) {
							LM_ERR("Either the pinger has died or BUG "
								   "found..\n");
							continue;
						}
						...
						as_nr--;
						LM_WARN("client [%.*s] leaving (Action Dispatcher "
								"Process died !)\n",
	     ...
	     * It will be done in the next iteration in if(seas_sigchld_received)*/
			...
			for(i = 0; i < 2; i++) {
				...
				if(poll_fds[i].revents & POLLIN) {
					...
					if((fd = new_as_connect(socks[i], i == 0 ? 'e' : 'a'))
							>= 0) {
			...
			for(i = 0; (i < as_nr) && poll_events; i++) {
				...
				if(poll_tmp->revents & POLLIN) {
					...
					switch(handle_as_data(poll_tmp->fd)) {
						...
						case 0: /* event_response received and processed*/
							break;
						default:
							LM_WARN("unknown return type from "
									"handle_as_data\n");
					}
				}
				if(clean_index || (poll_tmp->revents & POLLHUP)) {
					...
					if(!as) {
						LM_ERR("the leaving client was not found in the "
							   "as_list\n");
					}
				}
			}
		...
		struct ip_addr *address, unsigned short port, int *fd)
{
	...
	for(i = 0; i < 2; i++) {
		...
		optval = 1;
		if(setsockopt(fd[i], SOL_SOCKET, SO_REUSEADDR, (void *)&optval,
				   sizeof(optval))
				== -1) {
			LM_ERR("setsockopt (%s)\n", strerror(errno));
			goto error;
		}
		if((bind(fd[i], (struct sockaddr *)&(su.s), sizeof(struct sockaddr_in)))
				== -1) {
			LM_ERR("bind (%s)\n", strerror(errno));
			goto error;
		}
...
read_again:
	...
	if(thepointer.ptr->as == NULL || !thepointer.ptr->as->connected
			|| thepointer.ptr->as->type == CLUSTER_TYPE) {
	...
	tries = 0;
write_again:
	i = write(thepointer.ptr->as->u.as.event_fd, thepointer.ptr->msg + j,
			thepointer.ptr->len - j);
	if(i == -1) {
		switch(errno) {
			case EINTR:
...
static inline int add_new_as(int event_idx, int action_idx, struct as_entry *as)
{
	...
	if(use_ha) {
		if(jain_ping_timeout) {
			if(0 > init_pingtable(&the_as->jain_pings, jain_ping_timeout,
					   (jain_ping_timeout / jain_ping_period + 1)
							   * PING_OVER_FACTOR)) {
		...
		if(servlet_ping_timeout) {
			if(0 > init_pingtable(&the_as->servlet_pings, servlet_ping_timeout,
					   (servlet_ping_timeout / servlet_ping_period + 1)
							   * PING_OVER_FACTOR)) {
	...
	for(tmp = as_list; tmp; tmp = tmp->next) {
		...
		for(j = 0; j < tmp->u.cs.num; j++) {
			if(tmp->u.cs.as_names[j].len == the_as->name.len
					&& !memcmp(tmp->u.cs.as_names[j].s, the_as->name.s,
							the_as->name.len)) {
				if(tmp->u.cs.num == tmp->u.cs.registered) {
					LM_ERR("AS %.*s belongs to cluster %.*s which is already "
						   "completed\n",
							the_as->name.len, the_as->name.s, tmp->name.len,
							tmp->name.s);
					break;
				}
				tmp->u.cs.registered++;
...
static inline int send_sockinfo(int fd)
{
	...
	int k = 0, j;
	buffer[k++] =
			16; /*This used to be T_TABLE_POWER in Kamailio 1.0.1, now it is hardcoded in config.h*/
...
static int process_event_reply(as_p as)
{
	...
	ev_len = (as->ev_buffer.s[0] << 24) | (as->ev_buffer.s[1] << 16)
			 | (as->ev_buffer.s[2] << 8) | ((as->ev_buffer.s[3]) & 0xFF);
	type = as->ev_buffer.s[4];
	processor_id = as->ev_buffer.s[5];
	flags = (as->ev_buffer.s[6] << 24) | (as->ev_buffer.s[7] << 16)
			| (as->ev_buffer.s[8] << 8) | ((as->ev_buffer.s[9]) & 0xFF);
	...
	while(as->ev_buffer.len >= ev_len) {
		switch(type) {
			case BIND_AC:
				...
				process_bind_action(as, processor_id, flags,
						&as->ev_buffer.s[10], ev_len - 10);
				break;
			case UNBIND_AC:
				LM_DBG("Processing an UNBIND action from AS (length=%d): "
					   "%.*s\n",
						ev_len, as->name.len, as->name.s);
				process_unbind_action(as, processor_id, flags,
						&as->ev_buffer.s[10], ev_len - 10);
				break;
			default:
		...
		memmove(as->ev_buffer.s, &(as->ev_buffer.s[ev_len]),
				(as->ev_buffer.len) - ev_len);
		(as->ev_buffer.len) -= ev_len;
		if(as->ev_buffer.len > 10) {
			ev_len = (as->ev_buffer.s[0] << 24) | (as->ev_buffer.s[1] << 16)
					 | (as->ev_buffer.s[2] << 8)
					 | ((as->ev_buffer.s[3]) & 0xFF);
			type = as->ev_buffer.s[4];
			processor_id = as->ev_buffer.s[5];
			flags = (as->ev_buffer.s[6] << 24) | (as->ev_buffer.s[7] << 16)
					| (as->ev_buffer.s[8] << 8) | ((as->ev_buffer.s[9]) & 0xFF);
		...
		char *payload, int len)
{
	...
	*buffer = 0;
	...
	if(i == MAX_BINDS) {
		LM_ERR("No more bindings allowed. Ignoring bind request for processor "
			   "%d\n",
				processor_id);
		return -1;
	}
	...
	for(si = xxx_listen; si; si = si->next) {
		if(my_addr.af == si->address.af && my_addr.len == si->address.len
...
static int handle_unc_as_data(int fd)
{
	...
	if(unc_as_t[i].flags
			& HAS_NAME) { /*shouldn't happen, if it has a name, it shouldnt be in fdset[]*/
	...
	for(as = as_list; as; as = as->next) {
		if(as->name.len == len && !memcmp(name1, as->name.s, len)) {
			if(as->connected) {
				LM_WARN("AppServer trying to connect with a name already taken "
						"(%.*s)\n",
						len, name1);
				unc_as_t[i].valid = 0;
				unc_as_t[i].flags = 0;
	...
	if(!as) {
		LM_ERR("a client tried to connect which is not declared in config. "
			   "script(%.*s)\n",
				len, name1);
		unc_as_t[i].valid = 0;
		unc_as_t[i].flags = 0;
    ...
    * if 'i' is in the lower part, then look for an unc_as in the upper part*/
	...
	for(j = (i >= MAX_UNC_AS_NR ? 0 : MAX_UNC_AS_NR); j < k; j++)
		if(unc_as_t[j].valid && (unc_as_t[j].flags & HAS_NAME)
...
again:
	...
	switch(which) {
		...
		case 'a':
			for(i = MAX_UNC_AS_NR;
					(i < (2 * MAX_UNC_AS_NR)) && unc_as_t[i].valid; i++)
				;
			if(i == 2 * MAX_UNC_AS_NR) {
				LM_WARN("no more incomplete connections allowed\n");
				goto error;
