...
static int w_process_register_norpl(
...
static cmd_export_t cmds[] = {
		...
		{"cpl_process_register", (cmd_function)w_process_register, 0, 0, 0,
				REQUEST_ROUTE},
		{"cpl_process_register_norpl", (cmd_function)w_process_register_norpl,
				0, 0, 0, REQUEST_ROUTE},
		{0, 0, 0, 0, 0, 0}};
...
struct module_exports exports = {
		"cplc", DEFAULT_DLFLAGS, /* dlopen flags */
...
static int cpl_init(void)
{
	...
	if(timer_avp.s && timer_avp.len > 0) {
		...
		if(pv_get_avp_name(0, &(avp_spec.pvp), &cpl_env.timer_avp, &avp_type)
				!= 0) {
	...
	if(cpl_db_bind(&db_url, &db_table) < 0)
		goto error;
...
static inline int get_dest_user(struct sip_msg *msg, str *username, str *domain)
{
	...
	if(!msg->new_uri.s || parse_uri(msg->new_uri.s, msg->new_uri.len, &uri) < 0
		...
		if(parse_uri(msg->first_line.u.request.uri.s,
			...
			if((!msg->to
					   && ((parse_headers(msg, HDR_TO_F, 0) == -1) || !msg->to))
		...
		struct sip_msg *msg, char *str1, char *str2, char *str3)
{
	...
	if(get_user_script(&username, cpl_env.use_domain ? &domain : 0, &script,
			   &cpl_bin_col)
			== -1)
		goto error0;
	...
	/* set the flags */
	cpl_intr->flags =
			(unsigned int)((unsigned long)str1) | ((unsigned long)str2);
	/* build user AOR */
...
static inline int do_script_action(struct sip_msg *msg, int action)
{
	...
	switch(action) {
		case STORE_SCRIPT:
			...
			if(write_to_db(
					   &username, cpl_env.use_domain ? &domain : 0, &body, &bin)
					!= 1) {
				cpl_err = &intern_err;
				goto error_1;
			}
...
static inline int do_script_download(struct sip_msg *msg)
{
	...
	if(add_lump_rpl(msg, CONTENT_TYPE_HDR, CONTENT_TYPE_HDR_LEN, LUMP_RPL_HDR)
			== 0) {
	...
	if(mime && mime == (TYPE_APPLICATION << 16) + SUBTYPE_CPLXML) {
		...
		for(param = disp->params; param; param = param->next) {
			if(param->name.len == ACTION_PARAM_LEN
					&& !strncasecmp(
							param->name.s, ACTION_PARAM, ACTION_PARAM_LEN))
				break;
		}
		if(param == 0) {
		...
		if(param->body.len == STORE_ACTION_LEN
				&& !strncasecmp(
						param->body.s, STORE_ACTION, STORE_ACTION_LEN)) {
			...
			if(do_script_action(msg, STORE_SCRIPT) == -1)
				goto error;
		} else if(param->body.len == REMOVE_ACTION_LEN
				  && !strncasecmp(
						  param->body.s, REMOVE_ACTION, REMOVE_ACTION_LEN)) {
